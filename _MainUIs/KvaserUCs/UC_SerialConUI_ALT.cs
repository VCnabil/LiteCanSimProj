...
{
    public partial class UC_SerialConUI_ALT : UserControl
    {
        string _portSelectedName = "";
        public UC_SerialConUI_ALT()
        {
            _portSelectedName = G_DefaultPortName;
            InitializeComponent();
            SERIALMNGR_2.Instance.aPortHasOpened_orCloesdEVENT += Instance_aPortHasOpened_orCloesdEVENT;
        ...
        private void lstCOMPorts_DoubleClick(object sender, EventArgs e)
        {
            if (lstCOMPorts.SelectedItem != null)
            {
                ...
                SERIALMNGR_2.Instance.OpenPort(selectedPort, baudRate, handshake, rtsEnable);
        ...
        bool populatListbox()
        {
            string[] ports = SERIALMNGR_2.Instance.GetAvailablePortNames();
            if (ports == null)
            {
        ...
        private void Btn_YesAuto_Click(object sender, EventArgs e)
        {
            ...
            Handshake handshake = (Handshake)Enum.Parse(typeof(Handshake), comboBoxHandshakes.SelectedItem.ToString());
            bool rtsEnable = checkBox_RTS.Checked;
            SERIALMNGR_2.Instance.OpenPort(_portSelectedName, baudRate, handshake, rtsEnable);
        }
    }
}
