...
{
    public partial class UC_DP : UserControl
    {
        ...
        {
            deltaxValue = ClacXiEtha.Calc_OG_DeltaX(latA, lonA, latB, lonB);   //ClacXiEtha.Calc_DeltaX(uC_GPS_CMD.Get_gpsobj(), uC_GPS_ACT.Get_gpsobj());
            deltayValue = ClacXiEtha.Calc_OG_DeltaY(latA, lonA, latB, lonB);   //ClacXiEtha.Calc_DeltaY(uC_GPS_CMD.Get_gpsobj(), uC_GPS_ACT.Get_gpsobj());

            lbl_dx.Text = "OGdx: " + deltaxValue.ToString();
            lbl_dy.Text = "OGdy: " + deltayValue.ToString();

            headingDoubleForXIEtha = uC_360Heading.AngleValue;//  tb_heading.Value / 10; // tb_heading.Value % 360;


            lbl_Heading.Text = headingDoubleForXIEtha.ToString() + _degreeSymbol;
            ...
            //for testing ClacXiEtha.ComputeOffsetsXiEta_usingMatrix using lblXI2 and  lblETA2
        ...
        private void TimerLoop_Tick(object sender, EventArgs e)
        {
            ...
            lbl_ACT_latUint.Text = latACTuint.ToString();
            lbl_ACT_lonUint.Text = lonACTuint.ToString();
            lbl_CMD_latUint.Text = latCMDuint.ToString();
            lbl_CMD_lonUint.Text = lonCMDuint.ToString();
            ...
            if (cb_allow.Checked)
            {
                ...
                // uc_PointLatLonACT.Get_POINT_LAT().Get_me_COORD_x1E7();
                // KvsrManager.Instance.SendPGN_withStatus(0x09F8017F, BitConverter.GetBytes(uc_PointLatLonACT.Get_POINT_LAT().Get_me_COORD_x1E7()).Concat(BitConverter.GetBytes(uc_PointLatLonACT.Get_POINT_LON().Get_me_COORD_x1E7())).ToArray());

                //    KvsrManager.Instance.SendPGN_withStatus(0x09F8017F, BitConverter.GetBytes(uC_GPS_CMD.Get_gpsobj().LatFullUnint()).Concat(BitConverter.GetBytes(uC_GPS_CMD.Get_gpsobj().LonFullUnint())).ToArray());
